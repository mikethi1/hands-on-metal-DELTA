...
permanently brick the device via AVB fuse-burning — this check prevents that"
    ...
    if [ -z "$boot_img" ] || [ ! -f "$boot_img" ]; then
        ...
        if [ -n "$recovered_boot_img" ] && [ -f "$recovered_boot_img" ]; then
            ...
            ux_abort "Anti-rollback check: no valid boot image found after checking the configured image path and all fallback locations. Run boot image acquisition first."
        fi
    fi
